...
// 参照はSELECT文！
$sql = 'SELECT * FROM tozan_record_table WHERE user_id=:user_id';
...
//歩行距離の合計
$sql = 'SELECT  SUM(distance) FROM tozan_record_table WHERE user_id=:user_id';
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
$status = $stmt->execute();
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
//レコードの合計
$sql = 'SELECT  COUNT(*) FROM tozan_record_table WHERE user_id=:user_id';
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
$status = $stmt->execute();
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
FROM
    tozan_record_table WHERE user_id=:user_id';
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
$status = $stmt->execute();
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
